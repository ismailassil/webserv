...
void	ChunkParser::parseInnerBoundary( const string& body ) {
	...
	if ( filenamePos != string::npos && filenamePos < crPos ) {
		...
		outfile.open( "_downloads/" + chunkInfo.filename , ios::app | ios::binary ); // to be removed
		if ( !outfile.is_open() ) throw runtime_error("-- failed to open - " + chunkInfo.filename + " --");
...
void	ChunkParser::pushChunk( void ) {
	if ( bodyStatus.isFile ) {
		...
		ofstream nOutfile( "_downloads/" + chunkInfo.name , ios::app | ios::binary ); // to be removed
		nOutfile << chunkInfo.Chunk; // to be removed
		nOutfile.close(); // to be removed
		metaData.push_back(make_pair(chunkInfo.name, chunkInfo.Chunk)), chunkInfo.Chunk.clear();
...
bool	ChunkParser::parseHeadBody( void ) {
	if ( !bodyStatus.isheadLength ) {
		...
		char	*end;
		lengthInfo.chunkLength = static_cast<size_t>( strtol(number.c_str(), &end, 16) );
		chunkInfo.requestChunk.erase( 0, pos + 2 );
	}
	size_t	currentLength = lengthInfo.chunkLength > chunkInfo.requestChunk.length() ? chunkInfo.requestChunk.length() : lengthInfo.chunkLength;
...
bool	ChunkParser::parseBodyLength( void ) {
	...
	if ( lengthInfo.chunkLength == 2 && chunkInfo.requestChunk.substr(0, 2) == "\r\n" ) {
...
void	ChunkParser::parseBodyChunk( void ) {
	...
	if ( lengthInfo.chunkLength == 0 ) {
		bodyStatus.isbodySize = false;
		if ( bodyStatus.isFile ) {
			outfile.write( chunkInfo.BodyChunk.c_str(), chunkInfo.BodyChunk.length() );
			outfile.flush();
		} else if ( bodyStatus.isText ) {
	...
	while ( !chunkInfo.requestChunk.empty() ) {
		...
		if ( !bodyStatus.initDone ) {
			...
			if ( !bodyStatus.perm ) {
				...
				char *end;
				lengthInfo.chunkLength = static_cast<size_t>(strtol(chunkInfo.requestChunk.substr(0, pos).c_str(), &end, 16));
				if ( lengthInfo.chunkLength == 0 ) {
			...
			if ( bodyStatus.perm ) {
				size_t	sLength = lengthInfo.chunkLength > chunkInfo.requestChunk.length() ? chunkInfo.requestChunk.length() : lengthInfo.chunkLength;
				lengthInfo.chunkLength -= sLength;
				chunkInfo.BodyChunk.append(chunkInfo.requestChunk.substr(0, sLength));
				if ( !lengthInfo.chunkLength ) {
					...
					outfile.flush();
					chunkInfo.BodyChunk.clear();
				} else chunkInfo.requestChunk.erase(0, sLength);
			}
		}
	}
}
